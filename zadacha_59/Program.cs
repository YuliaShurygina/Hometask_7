...
{
    int[] summa = new int[someMatrix.GetLength(0)];
...
PrintMatrix(matrix);
Console.WriteLine();
int[] columnsSumma = FindSum(matrix);
PrintArray(columnsSumma);
int minimumSummaIndex = FindMinRow(columnsSumma);
Console.WriteLine($" Строка {minimumSummaIndex +1} имеет наименьшую сумму элементов ");