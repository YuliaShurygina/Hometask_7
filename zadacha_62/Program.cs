...
int[,] FillMatrix(int m, int n, int beginning, int end)
{
    ...
    for (int i = 0; i < m; i++)
    {
        for ( int j = 0; j < n; j++)
        {
            newMatrix[i,j] = new Random().Next(beginning, end);
        }
    }
    return newMatrix;
}
...
int[,] ColumnDeletion(int[,] matrix, int column)
{
...
int[,] RawDeletion(int[,] matrix, int raw)
{
    ...
    for (int j =0; j < matrix.GetLength(1);j++)
    {
        ...
        for (int i = raw; i< matrix.GetLength(0)-1; i++)
        {
...
int raw = indexes.Item1;
int[,] matrixWithoutColumn = ColumnDeletion(matrix, column);
int[,] matrixWithoutRow = RawDeletion(matrixWithoutColumn, raw);
