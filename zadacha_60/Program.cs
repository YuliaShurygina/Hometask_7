...
int [] UniquesElements(int[,] matrix)
{
    int[] uniquesElements = new int[AmountUniques(matrix)];
    int position = 0;
    for (int i = 0; i < matrix.GetLength(0); i++)
        for (int j = 0; j < matrix.GetLength(1); j++)
           if(ContainsBeforeIndexes(matrix, i, j, matrix[i,j]) == false)
               {
                   uniquesElements[position] = matrix[i,j];
                   position++;
               }
...
int[,] Frequencies(int[,] matrix)
{
    ...
    int[] uniquesElements = UniquesElements(matrix);
    //PrintArray1(uniquesElements);
    int row = 0;
    for (int i = 0; i < matrix.GetLength(0); i++)
        for (int j = 0; j < matrix.GetLength(1); j++)
           if(ContainsBeforeIndexes(matrix, i, j, matrix[i,j]) == false)
           {
               frequencies[row,0] = uniquesElements[row];
               frequencies[row,1]= CountElementMatrix(matrix, uniquesElements[row]);
               row++;
            }
